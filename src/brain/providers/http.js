...
export class MediaHttpClient {
  constructor({ timeoutMs, maximumBytes, logger, fetchImpl = globalThis.fetch }) {
  ...
  async #request(url, options) {
    ...
    try {
      ...
      if (error?.name === 'AbortError') {
        throw new MediaProviderError('Media request timed out.', { code: 'TIMEOUT', cause: error });
      }
  ...
  return new Promise((resolve, reject) => {
    ...
    request.on('response', (response) => {
      if (!response.statusCode || response.statusCode < 200 || response.statusCode >= 300) {
        response.resume();
        reject(new MediaProviderError(`Provider returned HTTP ${response.statusCode}.`, {
          code: 'HTTP_ERROR',
      ...
      if (Number.isFinite(declaredLength) && declaredLength > maximumBytes) {
        response.destroy();
        reject(new MediaProviderError('Media payload is too large.', { code: 'TOO_LARGE' }));
        return;
      }
      ...
      response.on('end', () => {
        ...
        if (buffer.length < minimumBytes) {
          reject(new MediaProviderError('Provider returned an empty or invalid media payload.', { code: 'INVALID_RESPONSE' }));
          return;
        }
        resolve({
          buffer,
